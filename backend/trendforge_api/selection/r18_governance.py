...
def evaluate_challenger(
    ...
    *,
    ...
    holdout_brier, _, holdout_r = _metrics(
        payload.holdout.probabilities,
        payload.holdout.outcomes,
        payload.holdout.net_r,
    )
    blockers: list[str] = []
    ...
    return ChallengerEvaluationV1(
        ...
        model_hash=payload.manifest.model_hash,
...
def build_promotion_review(
    *,
    manifest: ModelManifestV1,
    evaluation: ChallengerEvaluationV1,
    decision: Literal["APPROVE", "REJECT"],
    reviewer: str,
    reason: str,
    r16_validation_status: str,
    previous_record_hash: str | None = None,
    now: datetime | None = None,
) -> PromotionReviewV1:
    ...
    return PromotionReviewV1(
        ...
        reviewer=reviewer,
        reason=reason,
        r16_validation_status=r16_validation_status,
...
def assess_drift(
    *,
    ...
    previous_champion_hash: str | None = None,
    now: datetime | None = None,
) -> DriftAssessmentV1:
    ...
    rollback = (
        ...
        if breached and manifest.state == ModelState.CHAMPION
        else None
    )
    ...
    if latest_runs:
        ...
        if trading_date:
            dataset_age_days = max(
                0,
                (datetime.now(UTC).date() - date.fromisoformat(trading_date)).days,
            )
    return {
        "contract": CONTRACT,
        ...
        "blockers": tuple(r16.get("blockers") or ())
        + (() if approved else ("PIT_NOT_APPROVED",)),
