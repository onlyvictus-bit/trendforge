...
from .r18_history import (
    R_HIST_03D_AUTOMATIC_PROMOTION_ALLOWED,
    R_HIST_03D_EXECUTION_AUTHORIZED,
    ...
    build_frozen_dataset_member,
    build_governance_audit_record,
    build_governed_model_version,
    build_strategy_profile_version,
...
class ModelManifestV1(BaseModel):
    ...
    def immutable_identity(self) -> "ModelManifestV1":
        expected = _hash(self.model_id, self.model_version, self.purpose, self.market, self.horizon, self.dataset_hash, self.feature_set_hash, self.formula_set_hash)
...
class FoldPredictionV1(BaseModel):
    ...
    def validate_fold(self) -> "FoldPredictionV1":
        ...
        if not self.probabilities or not (len(self.probabilities) == len(self.outcomes) == len(self.net_r)):
...
def _metrics(probabilities: tuple[float, ...], outcomes: tuple[int, ...], net_r: tuple[float, ...]) -> tuple[float, float, float]:
    ...
    ece = sum(len(rows) / len(outcomes) * abs(sum(p for p, _ in rows) / len(rows) - sum(y for _, y in rows) / len(rows)) for rows in bins.values())
...
def evaluate_challenger(payload: ChallengerEvaluationInputV1, *, now: datetime | None = None) -> ChallengerEvaluationV1:
    ...
    holdout_brier, _, holdout_r = _metrics(payload.holdout.probabilities, payload.holdout.outcomes, payload.holdout.net_r)
    blockers: list[str] = []
    if payload.r16_validation_status != "PIT_APPROVED": blockers.append("PIT_NOT_APPROVED")
    if len(y) < 30: blockers.append("EFFECTIVE_SAMPLE_INSUFFICIENT")
    if average_r <= 0 or holdout_r <= 0: blockers.append("NET_EXPECTANCY_NOT_POSITIVE")
    if ece > 0.05: blockers.append("CALIBRATION_ERROR_EXCEEDS_POLICY")
    evaluated_at = now or datetime.now(UTC)
    evaluation_id = _hash(payload.manifest.model_hash, payload.dataset_hash, payload.cost_model_version, tuple(fold.model_dump(mode="json") for fold in folds), payload.holdout.model_dump(mode="json"), POLICY_VERSION)
    return ChallengerEvaluationV1(evaluation_id=evaluation_id, model_id=payload.manifest.model_id, model_hash=payload.manifest.model_hash, dataset_hash=payload.dataset_hash, evaluated_at=evaluated_at, fold_count=len(folds), observation_count=len(y), brier_score=brier, expected_calibration_error=ece, average_net_r_after_costs=average_r, holdout_brier_score=holdout_brier, holdout_average_net_r_after_costs=holdout_r, review_eligible=not blockers, blockers=tuple(blockers))


class PromotionReviewV1(BaseModel):
    model_config = ConfigDict(alias_generator=_to_camel, populate_by_name=True)
    review_id: str; model_id: str; model_hash: str; evaluation_id: str
    evidence_hash: str = Field(pattern=r"^[0-9a-f]{64}$")
    signature_scheme: Literal["SHA256_CHAIN_V1"] = "SHA256_CHAIN_V1"
    decision: Literal["APPROVE", "REJECT"]; reviewer: str; reason: str
    r16_validation_status: Literal["PIT_APPROVED", "PIT_NOT_APPROVED"]
    effective_state: ModelState; previous_record_hash: str | None = None
    record_hash: str; reviewed_at: datetime
...
def build_promotion_review(*, manifest: ModelManifestV1, evaluation: ChallengerEvaluationV1, decision: Literal["APPROVE", "REJECT"], reviewer: str, reason: str, r16_validation_status: str, previous_record_hash: str | None = None, now: datetime | None = None) -> PromotionReviewV1:
    if not reviewer.strip() or not reason.strip(): raise ValueError("HUMAN_REVIEWER_AND_REASON_REQUIRED")
    if decision == "APPROVE" and r16_validation_status != "PIT_APPROVED": raise ValueError("PIT_NOT_APPROVED")
    if decision == "APPROVE" and not evaluation.review_eligible: raise ValueError("CHALLENGER_NOT_REVIEW_ELIGIBLE")
    ...
    return PromotionReviewV1(review_id=review_id, model_id=manifest.model_id, model_hash=manifest.model_hash, evaluation_id=evaluation.evaluation_id, evidence_hash=evidence_hash, decision=decision, reviewer=reviewer, reason=reason, r16_validation_status=r16_validation_status, effective_state=effective, previous_record_hash=previous_record_hash, record_hash=record_hash, reviewed_at=reviewed_at)


class DriftAssessmentV1(BaseModel):
    model_config = ConfigDict(alias_generator=_to_camel, populate_by_name=True)
    drift_id: str; model_id: str; model_hash: str; assessed_at: datetime
    feature_psi: float = Field(ge=0); prediction_psi: float = Field(ge=0); calibration_delta: float = Field(ge=0)
    breached: tuple[str, ...]; resulting_state: ModelState
    rollback_model_hash: str | None = None; rollback_state: Literal["CHAMPION"] | None = None
    automatic_promotion: Literal[False] = False
...
def assess_drift(*, manifest: ModelManifestV1, feature_psi: float, prediction_psi: float, calibration_delta: float, previous_champion_hash: str | None = None, now: datetime | None = None) -> DriftAssessmentV1:
    ...
    if any(not math.isfinite(value) or value < 0 for value in values): raise ValueError("DRIFT_VALUE_INVALID")
    breached: list[str] = []
    if feature_psi >= 0.20: breached.append("FEATURE_DRIFT")
    if prediction_psi >= 0.20: breached.append("PREDICTION_DRIFT")
    if calibration_delta >= 0.05: breached.append("CALIBRATION_DRIFT")
    ...
    return DriftAssessmentV1(drift_id=_hash(manifest.model_hash, assessed_at.isoformat(), values, POLICY_VERSION), model_id=manifest.model_id, model_hash=manifest.model_hash, assessed_at=assessed_at, feature_psi=feature_psi, prediction_psi=prediction_psi, calibration_delta=calibration_delta, breached=tuple(breached), resulting_state=state, rollback_model_hash=rollback, rollback_state="CHAMPION" if rollback else None)


def governance_status() -> dict[str, object]:
    r16 = r16_status_payload(); approved = r16.get("validationStatus") == "PIT_APPROVED"
    latest_runs = r16_runs_payload(limit=1).get("runs") or []; dataset_age_days: int | None = None
    if latest_runs:
        trading_date = latest_runs[0].get("tradingDate")
        if trading_date: dataset_age_days = max(0, (datetime.now(UTC).date() - date.fromisoformat(trading_date)).days)
    return {"contract": CONTRACT, "modelState": "MODEL_NOT_APPROVED", "r16ValidationStatus": r16.get("validationStatus", "PIT_NOT_APPROVED"), "datasetStatus": r16.get("datasetStatus", "BUILDING"), "datasetAgeDays": dataset_age_days, "blockers": tuple(r16.get("blockers") or ()) + (() if approved else ("PIT_NOT_APPROVED",)), "probabilityVisible": False, "winRateVisible": False, "performanceVisible": False, "automaticPromotionAllowed": False, "executionAuthorized": False, "copy": "Research simulation only - no broker order."}
