...
def apply_schema() -> dict[str, Any]:
    ...
    try:
        ...
        """)
        conn.execute("INSERT OR IGNORE INTO schema_migrations(version, description, applied_at) VALUES (?, ?, ?)", (MIGRATION_VERSION, "R18 append-only model governance", datetime.now(UTC).isoformat()))
        conn.commit()
    finally:
        conn.close()
...
def _persist(table: str, record_id: str, payload: dict[str, Any], columns: tuple[str, ...], values: tuple[Any, ...]) -> bool:
    ...
    try:
        ...
        if existing:
            ...
            return False
        names = ("record_id",) + columns + ("payload_json", "content_hash", "created_at")
        marks = ",".join("?" for _ in names)
        conn.execute(f"INSERT INTO {table} ({','.join(names)}) VALUES ({marks})", (record_id,) + values + (encoded, content_hash, datetime.now(UTC).isoformat()))
        conn.commit()
        return True
    finally:
        conn.close()
...
def list_payloads(table: str, limit: int = 100) -> list[dict[str, Any]]:
    ...
    conn = storage.connect()
    try:
        rows = conn.execute(f"SELECT payload_json FROM {table} ORDER BY created_at DESC, record_id DESC LIMIT ?", (max(1, min(limit, 1000)),)).fetchall()
    finally:
        conn.close()
